 ...
 *  touchkeys
 *
 *  Created by Andrew McPherson on 3/8/11.
...
public:
    ...
    // ***** Data Logging *****
    void createLogFile(string keyTouchLog_filename, string path);
    void closeLogFile();
    void startLogging();
    void stopLogging();
	...
	void setLowestMidiNote(int note) {
		...
		lowestMidiNote_ = note;
		if(isOpen())
			keyboard_.setKeyboardRange(lowestMidiNote_, lowestMidiNote_ + 12*numOctaves_);
	...
	// ***** Run Loop Functions *****
    void* ledUpdateLoop();
	static void* staticLedUpdateLoop(void *arg) {
		return ((TouchkeyDevice*)arg)->ledUpdateLoop();
	}
	void* runLoop();
	static void* staticRunLoop(void *arg) {
		return ((TouchkeyDevice*)arg)->runLoop();
	}
    void* rawDataRunLoop();
    static void* staticRawDataRunLoop(void *arg) {
        return ((TouchkeyDevice*)arg)->rawDataRunLoop();
    }
    
    // for debugging
    void testStopLeds() { ledShouldStop_ = true; }
    
    /* User-defined callback methods for externally processing touch events */
    typedef void (*CentroidCallback) (timestamp_type timeStamp, int midiNote, KeyTouchFrame frame, void *userData);
    
    typedef void (*AnalogCallback) (timestamp_type timeStamp, int midiNote, float position, void *userData);
    
    void setCentroidCallback(CentroidCallback callback, void *userData);
    void setAnalogCallback(AnalogCallback callback, void *userData);
    
    void cancelCentroidCallback();
    void cancelAnalogCallback();
    void logMidiNoteOn(int midiNote, int velocity);
    void logMidiNoteOff(int midiNote);
...
private:
    ...
    // ***** Callbacks ***** //
    bool usingCentroidCallback_;
    bool usingAnalogCallback_;
    void *centroidCallback_;
    void *analogCallback_;
    void *centroidUserData_;
    void *analogUserData_;
};
